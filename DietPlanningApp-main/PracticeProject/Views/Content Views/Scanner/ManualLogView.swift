...
struct ManualLogView: View {
    ...
    private func saveToCalorieModel() {
            guard let adjustedCalories = adjustedCalories,
                  let adjustedProtein = adjustedProtein,
                  let adjustedTotalFat = adjustedTotalFat,
                  let adjustedSugar = adjustedSugar else {
                return
            }

            let userID = UserDefaults.standard.value(forKey: "UserID") as! String
            let calorieCount = Int(adjustedCalories)
            let proteinCount = Int(adjustedProtein)
            let fatsCount = Int(adjustedTotalFat)
            let carbsCount = Int(adjustedSugar)
            let metricQuantity = Int(adjustedmetricQty ?? 0)

            let calData = CalorieModel(
                userID: userID,
                name: foodName,
                calories: calorieCount,
                quantity: metricQuantity,
                protien: proteinCount,
                carbs: carbsCount,
                fats: fatsCount,
                calCount: calorieCount,
                protienCount: proteinCount,
            ...
            withAnimation {
                ...
                calCountDatabase.append(calData) // Update the binding array
            }
        }
    }
